...
func queryFavoriteByUserID(tx *gorm.DB, userID uint) ([]Favorite, error) {
	...
	if err := tx.Model(Favorite{}).Where("exist=1").Where("user_id=?", userID).Find(&Favorites).Error; err != nil {
		return nil, err
	}
...
func queryFavoriteByVideoID(tx *gorm.DB, videoID uint) ([]Favorite, error) {
	var Favorites []Favorite
	if err := tx.Model(Favorite{}).Where("exist=1").Where("video_id=?", videoID).Find(&Favorites).Error; err != nil {
		return nil, err
	}
	return Favorites, nil
}
...
func queryFavoriteByUserIDAndVideoID(tx *gorm.DB, userID, videoID uint) (*Favorite, error) {
	var Favorites Favorite
	if err := tx.Model(Favorite{}).Where("exist=1").Where("user_id=? and video_id=?", userID, videoID).Find(&Favorites).Error; err != nil {
		return nil, err
	}
	return &Favorites, nil
}
